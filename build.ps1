...
$upload = "";
if ($UploadPackage.IsPresent) {
	Write-Verbose "Enabling package upload."
	$upload = "-UploadPackage=true"
}
...
$expression = "$CAKE_EXE `"$Script`" -target=`"$Target`" -configuration=`"$Configuration`" -verbosity=`"$Verbosity`" $UseMono $UseDryRun $upload -Experimental"
